...
<div class="py-12">
    <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
        <div class="bg-white  overflow-hidden shadow-sm sm:rounded-lg dark:bg-zinc-900">
            <div class="p-6 text-gray-900 dark:text-gray-100">
                <!-- Header -->
                <div class="mb-6">
                    <h1 class="text-3xl font-bold text-gray-800 dark:text-gray-100">Edit Produk</h1>
                    <p class="text-gray-600 mt-1 dark:text-gray-400">Perbarui detail produk: {{ $product->name }}</p>
                </div>
                ...
                <form action="{{ route('products.update', $product) }}" method="POST">
                    ...
                    <div class="mb-6">
                        ...
                        <input type="text" name="name" id="name"
                            ...
                            value="{{ old('name', $product->name) }}" placeholder="Contoh: Laptop Dell Inspiron"
                            required>
                        @error('name')
                    ...
                    <div class="mb-6">
                        ...
                        <textarea name="description" id="description" rows="4"
                            ...
                            placeholder="Deskripsi detail produk (opsional)">{{ old('description', $product->description) }}</textarea>
                        @error('description')
                            <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="grid grid-cols-2 gap-6 mb-6">
                        ...
                        <div>
                            ...
                            <input type="number" name="price" id="price" step="0.01" min="0"
                                ...
                                value="{{ old('price', $product->price) }}" placeholder="100000" required>
                            @error('price')
                                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div>
                            ...
                            <input type="number" name="quantity" id="quantity" min="0"
                                ...
                                value="{{ old('quantity', $product->quantity) }}" placeholder="50" required>
                            @error('quantity')
                                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                            @enderror
                        </div>
                    ...
                    <div class="grid grid-cols-2 gap-6 mb-6">
                        <!-- SKU -->
                        <div>
                            <label for="sku" class="block text-gray-700 dark:text-gray-300 font-bold mb-2">SKU (Kode Produk) *</label>
                            <input type="text" name="sku" id="sku"
                                ...
                                value="{{ old('sku', $product->sku) }}" placeholder="SKU-0001-AB" required>
                            @error('sku')
                                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div>
                            ...
                            <select name="category" id="category"
                                ...
                                <option value="Elektronik" @selected(old('category', $product->category) == 'Elektronik')>Elektronik</option>
                                <option value="Pakaian" @selected(old('category', $product->category) == 'Pakaian')>Pakaian</option>
                                <option value="Makanan" @selected(old('category', $product->category) == 'Makanan')>Makanan</option>
                                <option value="Peralatan" @selected(old('category', $product->category) == 'Peralatan')>Peralatan</option>
                                <option value="Lainnya" @selected(old('category', $product->category) == 'Lainnya')>Lainnya</option>
                            </select>
                            @error('category')
                                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                            @enderror
